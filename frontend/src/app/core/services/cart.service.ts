...
export class CartService {
  ...
  updateQuantity(productId: number, quantity: number) {
    ...
    if (existing) {
      if (quantity > existing.product.stock) {
        existing.quantity = existing.product.stock;
      } else if (quantity <= 0) {
  ...
  getTotalAmount(): number {
    return this.cartItems.getValue().reduce((total, item) => total + (item.product.studentPrice || item.product.price || 0) * item.quantity, 0);
  }
}
