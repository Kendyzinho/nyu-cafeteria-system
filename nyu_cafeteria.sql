-- ============================================================
-- NYU Cafeteria System - Base de datos completa
-- ============================================================
-- INSTRUCCIONES PARA COMPAÑEROS:
-- 1. Abrir XAMPP y encender Apache + MySQL
-- 2. Abrir phpMyAdmin (http://localhost/phpmyadmin)
-- 3. Ir a la pestaña "Importar" (o "SQL")
-- 4. Seleccionar este archivo y ejecutar
-- 5. Crear el archivo .env en /backend con:
--      DB_HOST=localhost
--      DB_PORT=3306
--      DB_USERNAME=root
--      DB_PASSWORD=
--      DB_NAME=nyu_cafeteria
-- 6. Ejecutar: cd backend && npm install && npm run start:dev
-- 7. Ejecutar: cd frontend && npm install && npm run start
-- ============================================================
...
--
DROP DATABASE IF EXISTS `nyu_cafeteria`;
CREATE DATABASE `nyu_cafeteria` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
USE `nyu_cafeteria`;

-- --------------------------------------------------------
--
-- Tabla `user`
-- Entidad: UserEntity (user.entity.ts)
-- Columnas: id, nombre, apellido, email (unique), password, tipo
--
...
--
-- Tabla `menu`
-- Entidad: MenuEntity (menu.entity.ts)
-- Columnas: id, nombre, descripcion, precio, categoria, disponible, stockActual, fechaDisponible, image
--
...
INSERT INTO `menu` (`id`, `nombre`, `descripcion`, `precio`, `categoria`, `disponible`, `stockActual`, `fechaDisponible`, `image`) VALUES
(5, 'Bowl de Quinoa y Pollo Grill', 'Proteína premium, vegetales frescos y aderezo artesanal.', 7500.00, 'Almuerzos', 1, 50, '2026-04-24 00:00:00', 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?auto=format&fit=crop&w=300&q=80'),
(6, 'Wrap Vegetariano', 'Hummus, espinaca, tomate y falafel en tortilla de maíz.', 4000.00, 'Opciones Ligeras', 1, 1, '2026-04-24 00:00:00', 'https://images.unsplash.com/photo-1628840042765-356cda07504e?auto=format&fit=crop&w=300&q=80'),
(7, 'Café Latte Vainilla', 'Café de especialidad con leche texturizada y vainilla.', 2500.00, 'Bebidas', 1, 30, '2026-04-24 00:00:00', 'https://images.unsplash.com/photo-1541167760496-1628856ab772?auto=format&fit=crop&w=300&q=80');

-- --------------------------------------------------------
--
-- Tabla `stock`
-- Entidad: StockEntity (stock.entity.ts)
-- Columnas: id, menuItemId (unique), cantidad, umbralMinimo, ultimaActualizacion
--
...
CREATE TABLE `stock` (
  ...
  `umbralMinimo` int(11) NOT NULL DEFAULT 0,
...
--
-- Tabla `meal_plan`
-- Entidad: MealPlanEntity (meal-plan.entity.ts)
-- Columnas: id, nombre, descripcion, precio, tipo, activo, reqMatricula, reqResidencia
--
...
-- --------------------------------------------------------
--
-- Tabla `student_status`
-- Entidad: StudentStatusEntity (student-status.entity.ts)
-- Columnas: usuarioId (PK), matriculaActiva, residenciaActiva, planActivoId, ultimaActualizacion
-- Relaciones: FK → user(id) CASCADE, FK → meal_plan(id) SET NULL
--
...
--
-- Tabla `promotion`
-- Entidad: PromotionEntity (promotion.entity.ts)
-- Columnas: id, nombre, descripcion, descuento, fechaInicio, fechaFin, activa, reqMatricula, reqResidencia, tipoAplicacion, categoria, productosIds
--
...
--
-- Tabla `orders`
-- Entidad: OrderEntity (order.entity.ts)
-- Columnas: id, usuarioId, items (JSON), total, estado, fechaCreacion, horarioRetiro
-- Relaciones: FK → user(id) CASCADE
--
